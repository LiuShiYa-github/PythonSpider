...
def get_lagou(zw):
    ...
    for page in range(1, 6):
        url = "https://www.lagou.com/wn/jobs?pn={}&px=new&fromSearch=true&kd={}".format(page, urllib.parse.quote(zw))
        # url = "https://www.lagou.com/wn/jobs?gj=3%E5%B9%B4%E5%8F%8A%E4%BB%A5%E4%B8%8B&pn={}&px=new&fromSearch=true&kd={}".format(page, urllib.parse.quote(zw))
        logging.info(url)
        headers = {
        ...
        for num in range(1, 16):
            ...
            for li in li_list:
                ...
                logging.info("薪资：{}".format(name_list[0].strip()))
                # item['薪资'] = name_list[0].strip() if name_list else None
                salary.append(int(name_list[0].strip().split('-')[1].split('k')[0]))
                if_list.append(int(name_list[0].strip().split('-')[1].split('k')[0]))

        time.sleep(random.randint(1, 2))
    logging.info("职位的个数为：{}".format(num_posts))
    logging.info("总薪资为：{}".format(salary))
    total_salary = 0
    for sa in salary:
        total_salary += sa
    try:
        item['value'] = total_salary / num_posts
        item['name'] = zw
        return item
...
@app.route("/")
def index():
    info_list = [get_lagou(zw='python开发'), get_lagou(zw='go开发'), get_lagou(zw='Java开发'), get_lagou(zw='运维'),
                 get_lagou(zw='devops'), get_lagou(zw='产品经理'), get_lagou(zw='前端')]
    logging.info(info_list)
    return render_template('lagou.html', data=info_list)
